...
require_relative 'WebsiteCarbon_types'
...
class WebsiteCarbonSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = WebsiteCarbonHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = WebsiteCarbonHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, WebsiteCarbonFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    WebsiteCarbonUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = WebsiteCarbonHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = WebsiteCarbonHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = WebsiteCarbonHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => WebsiteCarbonError.new(
        "#{op}_allow",
        "WebsiteCarbonSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = WebsiteCarbonHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = WebsiteCarbonError.new(
        "graphql_error", "WebsiteCarbonSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = WebsiteCarbonSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
