...
class DataEntity
  ...
  def make
    opts = @_entopts.dup
    DataEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = WebsiteCarbonHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Data, Hash] the current Data data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = WebsiteCarbonHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Data fields)
  def match_get
  ...
  # Load a single Data.
  #
  # @param reqmatch [DataLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Data.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Data, Hash] the loaded Data; raises WebsiteCarbonError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = WebsiteCarbonHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
