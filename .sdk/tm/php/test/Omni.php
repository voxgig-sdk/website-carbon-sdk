...
//    only - a typed WebsiteCarbonContext there would make every ctx
...
final class WebsiteCarbonOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new WebsiteCarbonContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new WebsiteCarbonSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new WebsiteCarbonResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new WebsiteCarbonError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new WebsiteCarbonResponse($clean['response']);
    ...
    public static function err_from_map($m): ?WebsiteCarbonError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new WebsiteCarbonError(is_string($code) ? $code : '', $msg);
    }
