...
// WebsiteCarbon SDK feature test
//
...
class FtCtrl extends WebsiteCarbonControl
{
...
class FtClient
{
    ...
    private WebsiteCarbonUtility $utility;

    public function __construct(WebsiteCarbonUtility $utility, array $options)
    {
    ...
    public function get_utility(): WebsiteCarbonUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (WebsiteCarbonContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public WebsiteCarbonContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (WebsiteCarbonContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = WebsiteCarbonConfig::make_feature($name);
    ...
    private static function buildUrl(WebsiteCarbonSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof WebsiteCarbonError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new WebsiteCarbonResponse($fetched);
        }
    ...
    private function fail(WebsiteCarbonContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (WebsiteCarbonContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
