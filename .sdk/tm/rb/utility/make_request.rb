...
module WebsiteCarbonUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = WebsiteCarbonResponse.new({})
    ...
    if fetch_err
      ...
      response = WebsiteCarbonResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = WebsiteCarbonResponse.new(fetched)
    else
