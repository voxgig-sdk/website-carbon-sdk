...
from websitecarbon_sdk.core.control import WebsiteCarbonControl
...
from websitecarbon_sdk.core.helpers import get_ctx_prop, to_map
...
class WebsiteCarbonContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = WebsiteCarbonControl()
        ...
        if isinstance(sp, WebsiteCarbonSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, WebsiteCarbonResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, WebsiteCarbonResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return WebsiteCarbonError(code, msg, self)
