...
The Python SDK for the WebsiteCarbon API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Data(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.WebsiteCarbon(None).load(
    {"id": "test01"}, None
)
