...
class WebsiteCarbonTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            WebsiteCarbonTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = WebsiteCarbonTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = WebsiteCarbonTestRunner.getenv("WEBSITE_CARBON_TEST_LIVE")
        override = WebsiteCarbonTestRunner.getenv("WEBSITE_CARBON_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = WebsiteCarbonTestRunner.getenv(key)
        ...
        explain = WebsiteCarbonTestRunner.getenv("WEBSITE_CARBON_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["WEBSITE_CARBON_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if WebsiteCarbonTestRunner._test_control is not None:
            return WebsiteCarbonTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                WebsiteCarbonTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            WebsiteCarbonTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = WebsiteCarbonTestRunner.load_test_control()
...
def load_env_local():
    WebsiteCarbonTestRunner.load_env_local()


def env_override(m):
    return WebsiteCarbonTestRunner.env_override(m)


def entity_data(v):
    return WebsiteCarbonTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return WebsiteCarbonTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return WebsiteCarbonTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return WebsiteCarbonTestRunner.load_test_control()


def live_client_options():
    return WebsiteCarbonTestRunner.live_client_options()


def live_delay_ms():
    return WebsiteCarbonTestRunner.live_delay_ms()
