...
[MCP](https://modelcontextprotocol.io) server exposing the WebsiteCarbon SDK as
two agent tools — `website-carbon_list` and `website-carbon_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/website-carbon-mcp)
make build
...
claude mcp add --scope user website-carbon \
  -- /absolute/path/to/website-carbon-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./website-carbon-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// website-carbon_list: first page of records
{ "entity": "data" }
{ "entity": "data", "query": { } }

// website-carbon_load: one record by id
{ "entity": "data", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/website-carbon-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user website-carbon \
     -- "$PWD"/dist/*/website-carbon-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `website-carbon_list` and `website-carbon_load` tools now appear
   in new sessions. Ask the agent to *"list data using website-carbon"*
   and it calls `website-carbon_list` with `{"entity":"data"}`.
...
### Call the `website-carbon_list` tool
...
### Call the `website-carbon_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `website-carbon_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `website-carbon_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./website-carbon-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"website-carbon_load","arguments":{"entity":"data","query":{"id":1}}}}'
```
