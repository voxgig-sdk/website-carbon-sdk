...
//     (\`new WebsiteCarbonSDK(...)\` / \`WebsiteCarbonSDK.test(...)\` become
//     \`WebsiteCarbonSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'WebsiteCarbonSDK'
...
const TEST_SEED = {"entity":{"data":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new WebsiteCarbonSDK(...)\` and any
// \`WebsiteCarbonSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+WebsiteCarbonSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/WebsiteCarbonSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
