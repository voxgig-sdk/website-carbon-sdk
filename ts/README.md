...
`client.Data()` — each with a small set of operations (`load`)
...
```ts
try {
  const data = await client.Data().load({ byte: 1, green: 1 })
  console.log(data)
} catch (err) {
...
await entity.load({ byte: 1, green: 1 })
...
| `tester(testopts?, sdkopts?)` | `WebsiteCarbonSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `WebsiteCarbonSDK.test(testopts?, sdkopts?)` | `WebsiteCarbonSDK` | Create a test-mode client. |
...
| `client` | `client(): WebsiteCarbonSDK` | Return the parent SDK client. |
...
| `co2` | `Record<string, any>` | Object containing data relating to CO2 emissions from each page load. |
...
website-carbon/
├── src/
│   ├── WebsiteCarbonSDK.ts        # Main SDK class
...
await data.load({ byte: 1, green: 1 })
