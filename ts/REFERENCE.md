# WebsiteCarbon TypeScript SDK Reference

Complete API reference for the WebsiteCarbon TypeScript SDK.
...
```ts
new WebsiteCarbonSDK(options?: object)
```
...
#### `WebsiteCarbonSDK.test(testopts?, sdkopts?)`
...
**Returns:** `WebsiteCarbonSDK` instance in test mode.
...
Alias for `WebsiteCarbonSDK.test()`.
...
```ts
const result = await client.Data().load({ byte: 1, green: 1 })
```
...
Return the parent `WebsiteCarbonSDK` instance.
