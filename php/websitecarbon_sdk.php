...
class WebsiteCarbonSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = WebsiteCarbonHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = WebsiteCarbonHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!WebsiteCarbonFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, WebsiteCarbonFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return WebsiteCarbonUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = WebsiteCarbonHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = WebsiteCarbonHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = WebsiteCarbonHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new WebsiteCarbonError($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = WebsiteCarbonHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new WebsiteCarbonError("graphql_error",
                "WebsiteCarbonSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new WebsiteCarbonSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
