# WebsiteCarbon PHP SDK



The PHP SDK for the WebsiteCarbon API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Data()` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```php
$client = WebsiteCarbonSDK::test($testopts, $sdkopts);
```
...
| `co2` | `array` | Object containing data relating to CO2 emissions from each page load. |
...
```
php/
├── websitecarbon_sdk.php          -- Main SDK class
...
The main class (`websitecarbon_sdk.php`) exports the SDK class
