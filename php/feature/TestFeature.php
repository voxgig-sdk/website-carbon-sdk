...
// WebsiteCarbon SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class WebsiteCarbonTestFeature extends WebsiteCarbonBaseFeature
{
    ...
    public function init(WebsiteCarbonContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (WebsiteCarbonContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = WebsiteCarbonParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (WebsiteCarbonContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(WebsiteCarbonContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = WebsiteCarbonParam::call($ctx, $k);
