...
// WebsiteCarbon SDK retry feature
...
class WebsiteCarbonRetryFeature extends WebsiteCarbonBaseFeature
{
    ...
    public function init(WebsiteCarbonContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (WebsiteCarbonContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(WebsiteCarbonContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(WebsiteCarbonContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
