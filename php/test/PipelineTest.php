...
// WebsiteCarbon SDK pipeline test
//
...
{
    private static function utility(): WebsiteCarbonUtility
    {
        return new WebsiteCarbonUtility();
    }
    ...
    private static function ctx(array $over = []): WebsiteCarbonContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new WebsiteCarbonContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new WebsiteCarbonOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof WebsiteCarbonError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new WebsiteCarbonOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = WebsiteCarbonMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new WebsiteCarbonOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = WebsiteCarbonMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new WebsiteCarbonOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = WebsiteCarbonMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new WebsiteCarbonSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = WebsiteCarbonMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        ...
        [, $err] = WebsiteCarbonMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new WebsiteCarbonSpec([]),
            'response' => null,
            'result' => new WebsiteCarbonResult([]),
        ]);
        [, $err] = WebsiteCarbonMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new WebsiteCarbonSpec(['step' => 's']),
            'response' => new WebsiteCarbonResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new WebsiteCarbonResult([]),
        ]);
        [, $err] = WebsiteCarbonMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new WebsiteCarbonSpec(['step' => 's']),
            'response' => new WebsiteCarbonResponse(self::resp(200, ['v' => 1])),
            'result' => new WebsiteCarbonResult([]),
        ]);
        [, $err] = WebsiteCarbonMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new WebsiteCarbonSpec(['step' => 's']),
            'response' => new WebsiteCarbonResponse(self::resp(200, ['v' => 2])),
            'result' => new WebsiteCarbonResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        WebsiteCarbonMakeResponse::call($ctx);
    ...
    {
        $preset = new WebsiteCarbonResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new WebsiteCarbonSpec([]),
            'response' => new WebsiteCarbonResponse([]),
            'result' => new WebsiteCarbonResult([]),
        ...
        [$got, $err] = WebsiteCarbonMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new WebsiteCarbonResult([])]);
        [, $err] = WebsiteCarbonMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new WebsiteCarbonSpec([]), 'result' => null]);
        [, $err] = WebsiteCarbonMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new WebsiteCarbonSpec(['step' => 's']),
            'result' => new WebsiteCarbonResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new WebsiteCarbonOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = WebsiteCarbonMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new WebsiteCarbonSpec(['step' => 's']),
            'result' => new WebsiteCarbonResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new WebsiteCarbonOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = WebsiteCarbonMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new WebsiteCarbonResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new WebsiteCarbonSpec([]),
            'result' => new WebsiteCarbonResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = WebsiteCarbonMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = WebsiteCarbonMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new WebsiteCarbonError('boom', 'boom');
        $utility->fetcher = function (WebsiteCarbonContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new WebsiteCarbonSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = WebsiteCarbonMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (WebsiteCarbonContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = WebsiteCarbonMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(WebsiteCarbonResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        WebsiteCarbonMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (WebsiteCarbonContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new WebsiteCarbonSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = WebsiteCarbonMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new WebsiteCarbonResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new WebsiteCarbonSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = WebsiteCarbonMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = WebsiteCarbonMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = WebsiteCarbonMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new WebsiteCarbonResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, WebsiteCarbonDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new WebsiteCarbonResult(['ok' => false])]);
        $this->expectException(WebsiteCarbonError::class);
        WebsiteCarbonDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new WebsiteCarbonResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', WebsiteCarbonMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        WebsiteCarbonMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new WebsiteCarbonBaseFeature();
        $b = new WebsiteCarbonBaseFeature();
        WebsiteCarbonFeatureAdd::call($ctx, $a);
        WebsiteCarbonFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): WebsiteCarbonBaseFeature
    {
        $f = new WebsiteCarbonBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        WebsiteCarbonFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): WebsiteCarbonContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new WebsiteCarbonSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = WebsiteCarbonPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        WebsiteCarbonPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new WebsiteCarbonResponse(['headers' => null]),
            'result' => new WebsiteCarbonResult([]),
        ]);
        WebsiteCarbonResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        WebsiteCarbonResultBody::call($ctx);
