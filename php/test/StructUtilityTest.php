...
// WebsiteCarbon SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = WebsiteCarbonOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(WebsiteCarbonOmni::structfix($now, Struct::undef()))
                        !== json_encode(WebsiteCarbonOmni::structfix($val, Struct::undef()))) {
