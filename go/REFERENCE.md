# WebsiteCarbon Golang SDK Reference

Complete API reference for the WebsiteCarbon Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *WebsiteCarbonSDK`
...
#### `Data(data map[string]any) WebsiteCarbonEntity`
...
```go
result, err := client.Data(nil).Load(map[string]any{"byte": 1, "green": 1}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
