...
import (
	...
	omni "github.com/voxgig-sdk/website-carbon-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.WebsiteCarbonError); is {
				out := map[string]any{
					"name":    "WebsiteCarbonError",
...
func omniCtx(arg any, client *sdk.WebsiteCarbonSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.WebsiteCarbonSDK); is {
			client = live
